...
export function AuthPanel({ error }: { error?: string }) {
    const [mode, setMode] = useState<AuthMode>(
        error === "signup" || error === "exists" ? "sign-up" : "sign-in",
    );
    ...
    const tabClass = (tab: AuthMode) =>
        `h-11 rounded-md px-4 text-sm font-bold transition ${
            mode === tab
                ...
                : "text-zinc-500 hover:bg-zinc-200/70 hover:text-zinc-950"
        }`;
        ...
        <main className="flex min-h-[calc(100dvh-16rem)] w-full flex-col justify-center bg-background-soft text-zinc-950">
            <section className="mx-auto w-full max-w-md">
                ...
                <nav
                    ...
                    className="mb-5 grid grid-cols-2 rounded-md bg-zinc-200/70 p-1">
                    <button
                        className={tabClass("sign-in")}
                        onClick={() => setMode("sign-in")}
                        type="button">
                        Login
                    </button>
                    <button
                        className={tabClass("sign-up")}
                ...
                {mode === "sign-up" ? (
                    <form
                        action={signUp}
                        className="grid gap-3 border-t border-zinc-200 py-5"
                        key="sign-up-form">
                        <input
                            className="field"
                            name="name"
