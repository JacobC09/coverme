...
const jwtSecret =
    process.env.JWT_SECRET ?? "dev-only-change-me-before-real-users";
...
export async function verifyToken(token?: string) {
    try {
        ...
        if (data.iss !== issuer || data.exp < Math.floor(Date.now() / 1000))
            return null;
        return data;
    } catch {
        return null;
...
export async function verifyPassword(password: string, saved: string) {
    ...
    return (
        savedHash.length === testHash.length &&
        timingSafeEqual(savedHash, testHash)
    );
}
